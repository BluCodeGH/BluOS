...
u32int _kmalloc(u32int sz, u32int *phys) {
  if (heap_start != 0) { //Is heap initialised?
    ...
    return _kmalloc(sz, phys); //Recurse (we know there will be space now)
...
u32int _kmalloc_a(u32int sz, u32int *phys) {
  if (heap_start != 0) { //Is heap initialised
    ...
    fstart->pointer = heap_end;
    heap_end = (heap_header *)(fstart + 1);
    return _kmalloc_a(sz, phys); //Recurse (we know there will be space now)
  } else {
...
u32int kmalloc_a(u32int sz) {
  return _kmalloc_a(sz, 0);
}

u32int kmalloc_p(u32int sz, u32int *phys) {
  return _kmalloc(sz, phys);
}

u32int kmalloc_ap(u32int sz, u32int *phys) {
  return _kmalloc_a(sz, phys);
}
